...
<body background="image/forumPage.jpg" style="font-family:'微软雅黑';color:#575757">
	...
	<table align="right" style="font-size=18px;color:#D6D6D6">
  	...
  	<table style="margin-top:16px;margin-left:35px;font-size:18px;">
  		...
  		int id=Integer.parseInt(request.getParameter("classifyId"));%>
 
  		<%
  		if(id==2){
  			List<forum> list=new forumSelect().getAllForumChefdoeuvre();
    		allPage=(list.size()%10==0)?(list.size()/10):(list.size()/10+1);      	
  			...
  			<td><a href="creator.jsp?name=<%=name %>&page=1" target="_blank" style="color:#575757"><%=session.getAttribute("fr.getForumid()")%></a></td>
 					...
 					<tr><td colspan="3"><hr style="margin-left:40px;width: 1260px;color:#CCCCCC;"/> </td></tr>
  		 			<%}
 					}%>
 				
 	    <table style="margin-top:3px;"  align="center">
    	<tr>
    	<td style="width: 40px;"><a href="forumPageElse.jsp?classifyId=<%=id%>&page=1" >首页</a></td>
    	<%    	
    	for(int n=0;n<allPage;n++){
    		if((n+1)==p){%>
    			<td style="width: 15px;"><%=n+1%></td>
    		<%}else{%>    	
    			<td style="width: 15px;"><a href="forumPageElse.jsp?classifyId=<%=id%>&page=<%=n+1%>" id="nowPage">[<%=n+1%>]</a></td>    	
    		<%}
    	}%>
    	<td style="width: 40px;"><a href="forumPageElse.jsp?classifyId=<%=id%>&page=<%=allPage%>">尾页</a></td>
    	</tr>
    </table>
 				<%}%>
 		...
 		<%
  		if(id==3){
  			List<forum> list=new forumSelect().getAllForumNovel();
    		allPage=(list.size()%10==0)?(list.size()/10):(list.size()/10+1);      	
    			if(list.size()>0){
  					for(int i=(p-1)*10;i<list.size()&&i<p*10;i++){
  						forum fr=list.get(i);  		
    	...
    	<td style="width: 40px;"><a href="forumPageElse.jsp?classifyId=<%=id%>&page=<%=allPage%>">尾页</a></td>
 		...
 		<%
  		if(id==4){
  			List<forum> list=new forumSelect().getAllForumMind();
    		allPage=(list.size()%10==0)?(list.size()/10):(list.size()/10+1);      	
    			if(list.size()>0){
  					for(int i=(p-1)*10;i<list.size()&&i<p*10;i++){
  						forum fr=list.get(i);  		
    	...
    	<td style="width: 40px;"><a href="forumPageElse.jsp?classifyId=<%=id%>&page=<%=allPage%>">尾页</a></td>
 		...
 		<%
  		if(id==5){
  			List<forum> list=new forumSelect().getAllForumTechnology();
    		allPage=(list.size()%10==0)?(list.size()/10):(list.size()/10+1);      	
    			if(list.size()>0){
  					for(int i=(p-1)*10;i<list.size()&&i<p*10;i++){
  						forum fr=list.get(i);  		
    	...
    	<td style="width: 40px;"><a href="forumPageElse.jsp?classifyId=<%=id%>&page=<%=allPage%>">尾页</a></td>
 		...
 		<%
  		if(id==6){
  			List<forum> list=new forumSelect().getAllForumThesis();
    		allPage=(list.size()%10==0)?(list.size()/10):(list.size()/10+1);      	
    			if(list.size()>0){
  					for(int i=(p-1)*10;i<list.size()&&i<p*10;i++){
  						forum fr=list.get(i);  		
    	...
    	<td style="width: 40px;"><a href="forumPageElse.jsp?classifyId=<%=id%>&page=<%=allPage%>">尾页</a></td>
 		...
 		<%
  		if(id==7){
  			List<forum> list=new forumSelect().getAllForumAncient();
    		allPage=(list.size()%10==0)?(list.size()/10):(list.size()/10+1);      	
    			if(list.size()>0){
  					for(int i=(p-1)*10;i<list.size()&&i<p*10;i++){
  						forum fr=list.get(i);  		
    	...
    	<td style="width: 40px;"><a href="forumPageElse.jsp?classifyId=<%=id%>&page=<%=allPage%>">尾页</a></td>
 		...
 		<%
  		if(id==8){
  			List<forum> list=new forumSelect().getAllForumForeign();
    		allPage=(list.size()%10==0)?(list.size()/10):(list.size()/10+1);      	
    			if(list.size()>0){
  					for(int i=(p-1)*10;i<list.size()&&i<p*10;i++){
  						forum fr=list.get(i);  		
